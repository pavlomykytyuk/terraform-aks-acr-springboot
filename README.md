# Overview  
This project demonstrates how to create an Azure Container Registry (ACR) to securely store Docker images in a private container registry. Additionally, it provisions an Azure Kubernetes Service (AKS) cluster to host Kubernetes deployments.
...
1. Login into the Azure account using azure cli
    ...
    az login
    ```
2. Create a new service principal to grant permissions for managing Azure resources:Create a new service principal  
...
5. Add the following secrets in your GitHub Actions repository:
...
IMPORTANT! This approach using Service Principal do not use in production environment.
...
To see `acr_password`, use the command 
    ...
    ```
Create new secrets for login to ACR and run Application Deployment pipeline
    ```
      REGISTRY_USERNAME 
      REGISTRY_PASSWORD
     ```

10. The first thing to debug in your cluster is if your nodes are all registered correctly:
